...
void startPotholeDetection() {
  ...
  stopPotholeDetection(); // avoid duplicate listeners

  // --- GYROSCOPE (10 samples/second) ---
  ...
  // --- ACCELEROMETER (10 samples/second) ---
  accelSub = accelerometerEventStream()
      .throttleTime(const Duration(milliseconds: 100))
      .listen((a) async {
    if (isProcessing) return; // prevent overload
    isProcessing = true;
...
// --- STOP STREAMS TO AVOID MEMORY LEAK ---
void stopPotholeDetection() {
  accelSub?.cancel();
  gyroSub?.cancel();
}
